...
class cNetUtil
{
	public:
      ...
      static bool		Protocol_Init(bool is_internet);
		...
		static void		Address_To_String(LPSOCKADDR_IN p_address, char * str, uint32_t len,
								uint16_t & port);
		static LPCSTR	Address_To_String(uint32_t ip_address);
		static void		String_To_Address(LPSOCKADDR_IN p_address, LPCSTR str, uint16_t port);
      static void		Create_Unbound_Socket(SOCKET & sock);
      static bool		Create_Bound_Socket(SOCKET & sock, uint16_t port, SOCKADDR_IN & local_address);
      static void		Close_Socket(SOCKET & sock);
      static void		Create_Broadcast_Address(LPSOCKADDR_IN p_broadcast_address, uint16_t port);
      static void		Create_Local_Address(LPSOCKADDR_IN p_local_address, uint16_t port);
      static void		Broadcast(SOCKET & sock, uint16_t port, cPacket & packet);
      ...
      static uint32_t		Get_Default_Resend_Timeout_Ms() {return DefaultResendTimeoutMs;}
		...
		static const	uint16_t NETSTATS_SAMPLE_TIME_MS;
		static const	uint16_t KEEPALIVE_TIMEOUT_MS;
		static const	uint16_t MAX_RESENDS;
		static const	uint16_t MULTI_SENDS;
		static const	uint16_t RESEND_TIMEOUT_LAN_MS;
		static const	uint16_t RESEND_TIMEOUT_INTERNET_MS;
   ...
   private:
		static int		Get_Local_Tcpip_Addresses(SOCKADDR_IN ip_address[], uint16_t max_addresses);
      ...
      //static uint16_t Get_Header_Bytes() {return HeaderBytes;}
      //static uint16_t Get_Max_Packet_App_Data_Size() {return MaxPacketAppDataSize;}

      //static uint16_t HeaderBytes;
      //static uint16_t MaxPacketAppDataSize;

	   //static const uint16_t WS_VERSION_REQD;
