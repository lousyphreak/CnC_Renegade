...
RefPtr<LadderData> LadderData::Create(const WOL::Ladder& ladder, int32_t time)
	{
...
LadderData::LadderData(const WOL::Ladder& ladder, int32_t time) :
		mTimeStamp(time)
	{
...
bool LadderData::UpdateData(const WOL::Ladder& ladder, int32_t time)
	{
	if (time > mTimeStamp)
		{
		memcpy(&mData, &ladder, sizeof(mData));
...
LadderInfoEvent::LadderInfoEvent(const wchar_t* requested, const WOL::Ladder& ladder, int32_t time) :
