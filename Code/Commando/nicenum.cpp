...
uint16_t	cNicEnum::NumNics			= 0;
uint32_t		cNicEnum::GSNicList[];	
uint16_t	cNicEnum::NumGSNics		= 0;
...
)
{
	...
	{
		if ((uint32_t) cUserOptions::PreferredGameSpyNic.Get() == GSNicList[index])
		{
			is_nic_valid = true;
			break;
		}
