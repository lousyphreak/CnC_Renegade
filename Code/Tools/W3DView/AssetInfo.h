...
class AssetInfoClass
{
	public:
		...
		AssetInfoClass (LPCTSTR passet_name, ASSET_TYPE type, RenderObjClass *prender_obj = NULL, uint32_t user_data = 0L)
			: m_Name (passet_name),
			  m_AssetType (type),
		...
		uint32_t					Get_User_Number (void) const				{ return m_dwUserData; }
		...
		void					Set_User_Number (uint32_t user_data)		{ m_dwUserData = user_data; }
