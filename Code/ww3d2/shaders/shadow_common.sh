...
float ComputeShadow(vec3 worldPos, vec3 worldNormal, float viewDepth)
{
    ...
    if (normalBias > 0.0) {
        shadowWorldPos += normalize(worldNormal) * normalBias;
    }

    vec3 shadowUV = WorldToShadowUV(shadowWorldPos, cascade);
    ...
    float shadow = SampleShadowPCF(shadowUV, cascade, u_shadowConfig.y);
