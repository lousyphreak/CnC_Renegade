...
int Stack_Walk(uint32_t *return_addresses, int num_addresses, CONTEXT *context = nullptr);
bool Lookup_Symbol(void *code_ptr, char *symbol, int &displacement);
void Load_Image_Helper(void);
void Register_Thread_ID(uint32_t thread_id, char *thread_name, bool main = false);
...
*/
extern uint32_t ExceptionReturnStack;
extern uint32_t ExceptionReturnAddress;
extern uint32_t ExceptionReturnFrame;
