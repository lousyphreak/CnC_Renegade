...
const uint32_t ParticleBufferClass::PermutationArray[16] = {
	11, 3, 7, 14, 0, 13, 1, 2, 5, 12, 15, 6, 9, 8, 4, 10
};

// Maximum size of randomizer tables
const static uint32_t MAX_RANDOM_ENTRIES = 32;	// MUST be power of two!

// Total Active Particle Buffer Count
uint32_t ParticleBufferClass::TotalActiveCount = 0;
...
namespace
{
const char *Particle_Debug_Get_Emitter_Name(const ParticleEmitterClass *emitter)
{
	if (emitter == NULL) {
		return "<null>";
	}
	...
	return (name != NULL && name[0] != 0) ? name : "<unnamed>";
}

bool Particle_Debug_Is_Ground_Effect_Emitter(const ParticleEmitterClass *emitter)
{
	const char *name = Particle_Debug_Get_Emitter_Name(emitter);
	return
		std::strstr(name, "roll") != NULL ||
		std::strstr(name, "sld") != NULL ||
		std::strstr(name, "dust") != NULL ||
		std::strstr(name, "e_tr_") != NULL ||
		std::strstr(name, "e_tk_") != NULL;
}

bool Particle_Debug_Should_Sample_Log(const ParticleEmitterClass *emitter)
{
	return Particle_Debug_Is_Ground_Effect_Emitter(emitter) && ((WW3D::Get_Frame_Count() & 0x1F) == 0);
}
}
	...
	DefaultTailDiffuse(0,0,0,0)
{
	...
	APT = NEW_REF( ShareBufferClass<uint32_t> , (MaxNum) );
	Velocity = new Vector3[MaxNum]; 
	TimeStamp = new uint32_t[MaxNum];
	...
	DefaultTailDiffuse(src.DefaultTailDiffuse)
{
	...
	*/
	...
	if (src.Color) {
		...
		ColorKeyFrameTimes = new uint32_t [NumColorKeyFrames];
		...
		if (src.RandomColorEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomColorEntriesMinus1; j++) {
	...
	if (src.Alpha) {
		...
		AlphaKeyFrameTimes = new uint32_t [NumAlphaKeyFrames];
		...
		if (src.RandomAlphaEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomAlphaEntriesMinus1; j++) {
	...
	if (src.Size) {
		...
		// Copy size keyframes
		SizeKeyFrameTimes = new uint32_t [NumSizeKeyFrames];
		...
		if (src.RandomSizeEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomSizeEntriesMinus1; j++) {
	...
	if (src.Orientation) {
		...
		RotationKeyFrameTimes = new uint32_t [NumRotationKeyFrames];
		...
		if (src.RandomRotationEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomRotationEntriesMinus1; j++) {
		...
		if (src.RandomOrientationEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomOrientationEntriesMinus1; j++) {
	...
	if (src.Frame || src.UCoord) {
		...
		FrameKeyFrameTimes = new uint32_t [NumFrameKeyFrames];
		...
		if (src.RandomFrameEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomFrameEntriesMinus1; j++) {
	...
	if (NumBlurTimeKeyFrames > 0) {
		...
		BlurTimeKeyFrameTimes = new uint32_t [NumBlurTimeKeyFrames];
		...
		if (src.RandomBlurTimeEntries) {
			...
			for (uint32_t j = 0; j <= NumRandomBlurTimeEntriesMinus1; j++) {
...
void ParticleBufferClass::Generate_APT(ShareBufferClass <uint32_t> **apt,uint32_t &active_point_count)
{
	if (NonNewNum < (int)MaxNum || DecimationThreshold > 0) {
		...
		uint32_t sub1_start;	// Start of subrange 1.
		uint32_t sub1_end;		// End of subrange 1.
		uint32_t sub2_start;	// Start of subrange 2.
		uint32_t sub2_end;		// End of subrange 2.
		...
		// Generate APT:
		uint32_t *apt_ptr = APT->Get_Array();
		...
		if (active_point_count == 0 && NonNewNum > 0 && DecimationThreshold > 0 && DecimationThreshold < (LodCount - 1)) {
			apt_ptr[active_point_count++] = Start;

			if (Particle_Debug_Is_Ground_Effect_Emitter(Emitter)) {
				WWRELEASE_SAY((
					"PARTICLE_APT_FALLBACK emitter=%s particles=%d start=%u lod=%d threshold=%u projected_area=%.3f max_particles=%u\n",
					Particle_Debug_Get_Emitter_Name(Emitter),
					NonNewNum,
					Start,
					Get_LOD_Level(),
					DecimationThreshold,
...
void ParticleBufferClass::Render_Particles(RenderInfoClass & rinfo)
{
	...
	TextureClass *texture = PointGroup->Peek_Texture();
	const ShaderClass shader = PointGroup->Get_Shader();
	if (Particle_Debug_Should_Sample_Log(Emitter) || (Particle_Debug_Is_Ground_Effect_Emitter(Emitter) && (active_point_count == 0 || texture == NULL))) {
		const char *texture_name = "<null>";
		if (texture != NULL) {
			texture_name = texture->Get_Texture_Name();
		}
		WWRELEASE_SAY((
			"PARTICLE_RENDER_TRACE emitter=%s mode=%d particles=%d active=%u lod=%d threshold=%u projected_area=%.3f size=%.3f alpha=%.3f tex=%s shader=0x%08X blend=%d/%d alpha_test=%d depth_compare=%d depth_mask=%d\n",
			Particle_Debug_Get_Emitter_Name(Emitter),
			RenderMode,
			Get_Particle_Count(),
			active_point_count,
...
void ParticleBufferClass::Prepare_LOD(CameraClass &camera)
{
	...
	const int previous_lod = Get_LOD_Level();
	...
	const int new_lod = Get_LOD_Level();
	if (Particle_Debug_Is_Ground_Effect_Emitter(Emitter) && (Particle_Debug_Should_Sample_Log(Emitter) || previous_lod != new_lod || new_lod <= 1)) {
		WWRELEASE_SAY((
			"PARTICLE_LOD_TRACE emitter=%s particles=%d projected_area=%.3f min_lod=%d lod=%d threshold=%u max_particles=%u max_size=%.3f\n",
			Particle_Debug_Get_Emitter_Name(Emitter),
			Get_Particle_Count(),
			ProjectedArea,
			minlod,
...
void ParticleBufferClass::Reset_Colors(ParticlePropertyStruct<Vector3> &new_props)
{
	...
	uint32_t i;	// Used in loops
	uint32_t ui_previous_key_time = 0;
	uint32_t ui_current_key_time = 0;
	...
	if (color_rand_zero && new_props.NumKeyFrames == 0) {
		...
		for (ckey = 0; ckey < new_props.NumKeyFrames; ckey++) {
			ui_current_key_time = (uint32_t)(new_props.KeyTimes[ckey] * 1000.0f);
		...
		uint32_t new_num_color_key_frames = ckey + 1;// Includes start keyframe (keytime == 0).
		...
		for (i = 1; i < NumColorKeyFrames; i++) {
			uint32_t im1 = i - 1;
			ColorKeyFrameTimes[i] = (uint32_t)(new_props.KeyTimes[im1] * 1000.0f);
		...
		if (color_rand_zero) {
			...
			uint32_t pot_num = Find_POT(MaxNum);
			uint32_t default_randomizer_entries = MIN(pot_num, MAX_RANDOM_ENTRIES);
...
void ParticleBufferClass::Reset_Opacity(ParticlePropertyStruct<float> &new_props)
{
	...
	if (alpha_rand_zero && new_props.NumKeyFrames == 0) {
		...
		for (akey = 0; akey < new_props.NumKeyFrames; akey++) {
			ui_current_key_time = (uint32_t)(new_props.KeyTimes[akey] * 1000.0f);
			WWASSERT(ui_current_key_time > ui_previous_key_time);
			if (ui_current_key_time >= MaxAge) break;
			ui_previous_key_time = ui_current_key_time;
		}
		...
		// otherwise release and reallocate.
		uint32_t new_num_alpha_key_frames = akey + 1;// Includes start keyframe (keytime == 0).
		...
		for (i = 1; i < NumAlphaKeyFrames; i++) {
			uint32_t im1 = i - 1;
			AlphaKeyFrameTimes[i] = (uint32_t)(new_props.KeyTimes[im1] * 1000.0f);
...
void ParticleBufferClass::Reset_Size(ParticlePropertyStruct<float> &new_props)
{
	...
	if (size_rand_zero && new_props.NumKeyFrames == 0) {
		...
		for (skey = 0; skey < new_props.NumKeyFrames; skey++) {
			ui_current_key_time = (uint32_t)(new_props.KeyTimes[skey] * 1000.0f);
			WWASSERT(ui_current_key_time > ui_previous_key_time);
			if (ui_current_key_time >= MaxAge) break;
			ui_previous_key_time = ui_current_key_time;
		}
		...
		// otherwise release and reallocate.
		uint32_t new_num_size_key_frames = skey + 1;// Includes start keyframe (keytime == 0).
		...
		for (i = 1; i < NumSizeKeyFrames; i++) {
			uint32_t im1 = i - 1;
			SizeKeyFrameTimes[i] = (uint32_t)(new_props.KeyTimes[im1] * 1000.0f);
	...
	*/
	...
	if (orientation_rand_zero && rotation_rand_zero && new_props.NumKeyFrames == 0 && fabs(new_props.Start) < eps_rotation) {
		...
		for (key = 0; key < new_props.NumKeyFrames; key++) {
			ui_current_key_time = (uint32_t)(new_props.KeyTimes[key] * 1000.0f);
			WWASSERT(ui_current_key_time > ui_previous_key_time);
			if (ui_current_key_time >= MaxAge) break;
			ui_previous_key_time = ui_current_key_time;
		}
		...
		uint32_t new_num_key_frames = key + 1;// Includes start keyframe (keytime == 0).
		...
		for (i = 1; i < NumRotationKeyFrames; i++) {
			uint32_t im1 = i - 1;
			RotationKeyFrameTimes[i] = (uint32_t)(new_props.KeyTimes[im1] * 1000.0f);
   ...
   float oo_intmax = 1.0f / (float)INT_MAX;
	...
	if (frame_rand_zero && new_props.NumKeyFrames == 0) {
		...
		for (i = 1; i < NumFrameKeyFrames; i++) {
			uint32_t im1 = i - 1;
			FrameKeyFrameTimes[i] = (uint32_t)(new_props.KeyTimes[im1] * 1000.0f);
   ...
   float oo_intmax = 1.0f / (float)INT_MAX;
	...
	if (blurtime_rand_zero && new_blur_times.NumKeyFrames == 0) {
		...
		for (key = 0; key < new_blur_times.NumKeyFrames; key++) {
			ui_current_key_time = (uint32_t)(new_blur_times.KeyTimes[key] * 1000.0f);
			WWASSERT(ui_current_key_time > ui_previous_key_time);
			if (ui_current_key_time >= MaxAge) break;
			ui_previous_key_time = ui_current_key_time;
		}
		...
		for (i = 1; i < NumBlurTimeKeyFrames; i++) {
			uint32_t im1 = i - 1;
			BlurTimeKeyFrameTimes[i] = (uint32_t)(new_blur_times.KeyTimes[im1] * 1000.0f);
...
void ParticleBufferClass::Update_Kinematic_Particle_State(void)
{
	...
	uint32_t elapsed = WW3D::Get_Sync_Time() - LastUpdateTime;
...
void ParticleBufferClass::Update_Visual_Particle_State(void)
{
	...
	uint32_t current_time = WW3D::Get_Sync_Time();

	// The following back-to-back pair of "for" loops traverses the circular
	// buffer subranges in proper order.
	uint32_t ckey = NumColorKeyFrames - 1;
	uint32_t akey = NumAlphaKeyFrames - 1;
	uint32_t skey = NumSizeKeyFrames - 1;
	uint32_t rkey = NumRotationKeyFrames - 1;
	uint32_t fkey = NumFrameKeyFrames - 1;
	uint32_t bkey = NumBlurTimeKeyFrames -1;
	...
	for (part = Start; part < sub1_end; part++) {
		...
		uint32_t part_age = current_time - TimeStamp[part];
...
void ParticleBufferClass::Get_New_Particles(void)
{
	...
	for (; NewParticleQueueCount;) {
		...
		uint32_t age = current_time - TimeStamp[NewEnd];
...
void ParticleBufferClass::Update_Non_New_Particles(uint32_t elapsed)
{
	// In the general case, a range in a circular buffer can be composed of up
	// to two subranges. Find the Start - End subranges.
	uint32_t sub1_end;		// End of subrange 1.
