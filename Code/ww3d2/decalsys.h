...
protected:
	...
	*/
	virtual uint32_t							Generate_Decal_Id(void) { return Generate_Unique_Global_Decal_Id(); }
	...
	*/
	static uint32_t							Generate_Unique_Global_Decal_Id(void);

	static uint32_t							DecalIDGenerator;
};
...
public:
	...
	*/
	uint32_t							Get_Decal_ID(void)								{ return DecalID; }
...
protected:
	...
	DecalGeneratorClass(uint32_t id,DecalSystemClass * system);
	...
	*/
	...
	uint32_t							DecalID;								// unique ID generated by the DecalSystem
...
public:
	...
	MultiFixedPoolDecalSystemClass(uint32_t num_pools, const uint32_t *pool_sizes);
...
protected:
	...
	*/
	virtual	uint32_t	Generate_Decal_Id(void) { return encode_decal_id(Generator_PoolID, Generator_SlotID); }
	uint32_t	Generator_PoolID;		// These should be set before calling Lock_Decal_Generator()
	uint32_t	Generator_SlotID;		// These should be set before calling Lock_Decal_Generator()

	class LogicalDecalClass;

	// Get a reference to the logical decal at the given pool and slot id (performs range chacking)
	LogicalDecalClass & find_logical_decal(uint32_t pool_id, uint32_t slot_id);

	// Get a reference to the logical decal with the given decal id
	LogicalDecalClass & find_logical_decal(uint32_t decal_id);
	...
	static uint32_t encode_decal_id(uint32_t pool_id, uint32_t slot_id) { return (slot_id & 0xFFFF) | (pool_id << 16); }
	static void decode_decal_id(uint32_t decal_id, uint32_t & pool_id, uint32_t & slot_id) { slot_id = decal_id & 0xFFFF; pool_id = decal_id >> 16; }
