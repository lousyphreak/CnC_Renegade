...
bool SubTitleManagerClass::Process(uint32_t movieTime)
{
	if (mSubTitles == NULL) {
		return false;
	}
	...
	for (;;) {
		...
		uint32_t displayTime = subtitle->Get_Display_Time();
...
//			WWDEBUG_SAY(("SubTitle: %04d @ %u\n", mSubTitleIndex, movieTime));
	...
	if (mActiveSubTitle != NULL) {
		...
		uint32_t expireTime = subtitle->Get_Display_Time() + subtitle->Get_Display_Duration();
...
void SubTitleManagerClass::Draw_Sub_Title(const SubTitleClass* subtitle)
{
	...
	const WCHAR *string = reinterpret_cast<const WCHAR *>(subtitle->Get_Caption());
	...
	// Set font color
	uint32_t rgbColor = subtitle->Get_RGB_Color()|0xff000000;
