...
// ----------------------------------------------------------------------------
//
// Sorts an array. Uses values from array 'keys' as sort keys.
//
// ----------------------------------------------------------------------------
	...
	int count)		// array element count
{
	...
	else QuickSort(array,keys,0,count-1);
}
	...
	uint16_t vertex_count)
{
	...
	// D3DXVec3Transform computes result = vec * mtx (row-vector convention):
	//   result[j] = vec.x*mtx[0][j] + vec.y*mtx[1][j] + vec.z*mtx[2][j] + mtx[3][j]
	Matrix4 mtx = state->sorting_state.world * state->sorting_state.view;
	const Vector3& vec = state->bounding_sphere.Center;
	float tx = vec.X*mtx[0][0] + vec.Y*mtx[1][0] + vec.Z*mtx[2][0] + mtx[3][0];
	float ty = vec.X*mtx[0][1] + vec.Y*mtx[1][1] + vec.Z*mtx[2][1] + mtx[3][1];
	float tz = vec.X*mtx[0][2] + vec.Y*mtx[1][2] + vec.Z*mtx[2][2] + mtx[3][2];
	state->transformed_center=Vector3(tx,ty,tz);
...
static void Release_Refs(SortingNodeStruct* state)
{
...
void SortingRendererClass::Flush_Sorting_Pool()
{
	...
	DynamicVBAccessClass dyn_vb_access(BUFFER_TYPE_DYNAMIC_DX8,dynamic_fvf_type,overlapping_vertex_count);
	{
		...
		for (node_id=0;node_id<overlapping_node_count;++node_id) {
			...
			// Compute world*view, then extract Z row for depth sorting.
			// The original D3D code did: mtx = world*view; transpose(mtx); z = mtx[2] dot (x,y,z,1)
			// After transpose, mtx_t[2][j] = mtx[j][2], so:
			//   z = mtx[0][2]*x + mtx[1][2]*y + mtx[2][2]*z + mtx[3][2]
			Matrix4 wv = state->sorting_state.world * state->sorting_state.view;
			for (unsigned i=0;i<state->vertex_count;++i,++src_verts) {
				vertex_z_array[i] = (wv[0][2] * src_verts->x + wv[1][2] * src_verts->y + wv[2][2] * src_verts->z + wv[3][2]);
