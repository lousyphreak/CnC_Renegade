...
class BitStreamClass : public cBitPacker
{
	public:
      ...
      //
      void Add_Raw_Data(LPCSTR data, uint16_t data_size);
      ...
      void Add_Wide_Terminated_String(const WCHAR *string, bool permit_empty = false);
		void Get_Wide_Terminated_String (WCHAR *buffer, uint16_t buffer_len, bool permit_empty = false);		
...
#pragma auto_inline(off)
      ...
      template<class T> T Internal_Get(T & value, int type = NO_ENCODER) {
		...
		void Internal_Add_Wire32(uint32_t value, int type = NO_ENCODER) { Internal_Add(value, type); }
		uint32_t Internal_Get_Wire32(uint32_t & value, int type = NO_ENCODER) {
			uint32_t raw_value = 0;
