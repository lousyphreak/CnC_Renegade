...
class Dictionary
{
 public:
  ...
  int8_t             iterate(INOUT int &index,INOUT int &offset, OUT V &value) const;
...
template <class K,class V>
int8_t Dictionary<K,V>::contains(IN K &key)
{
  int offset;
  DNode<K,V> *node;
...
template <class K,class V>
int8_t Dictionary<K,V>::getValue(IN K &key,OUT V &value)
{
  int offset;
  DNode<K,V> *node;
