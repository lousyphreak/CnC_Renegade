...
/*
** Global Instance of the DX8MeshRenderer
*/
...
/**
** PolyRenderTaskClass
** This is a record of a polygon renderer that needs to be rendered
...
void DX8RigidFVFCategoryContainer::Log(bool only_visible)
{
	...
	if (vertex_buffer) {
		...
		work.Format("VB size (used/total): %d/%d FVF: %s\n",used_vertices,vertex_buffer->Get_Vertex_Count(),(const char*)fvfname);
		WWDEBUG_SAY((work));
	}
	else {
...
void DX8RigidFVFCategoryContainer::Render(void)
{
	...
	int zbias=0;
	DX8Wrapper::Set_DX8_Render_State(D3DRS_ZBIAS,zbias);
	for (unsigned p=0;p<passes;++p) {
		SNAPSHOT_SAY(("Pass: %d\n",p));
...
public:
		...
		allocated_polygon_array(false)
	{
		...
		// NPatches are not supported in the bgfx renderer
		npatch_enable = false;
	...
	*/
	if (!vertex_buffer) {
		...
		if (sorting) {
			...
			WWASSERT(vertex_buffer->FVF_Info().Get_FVF()==FVF);
		}
		else {
			vertex_buffer=NEW_REF(DX8VertexBufferClass,(FVF, vb_size));
		}
	}
	...
	*/
	if (!index_buffer) {
		...
		if (sorting) {
			...
			index_buffer=NEW_REF(DX8IndexBufferClass,(ib_size));
		}
	}
...
void DX8SkinFVFCategoryContainer::Render(void)
{
	...
	DX8Wrapper::Set_Vertex_Buffer((VertexBufferClass*)NULL);
	...
	unsigned pass)
{
	...
	*/
	...
	/*
	** Add the indices for the polygons that match into this renderer's index table.
	*/
	if (polygons) {
...
void DX8MeshRendererClass::Flush(void)
{
	...
	** rendered *after* the rigid meshes.
	*/
	for (i=0;i<texture_category_container_lists_rigid.Count();++i) {
	...
	*/
	...
	DX8Wrapper::Set_Vertex_Buffer((VertexBufferClass*)NULL);
	DX8Wrapper::Set_Index_Buffer((IndexBufferClass*)NULL,0);
}
