...
typedef struct {
	...
	void ( * Modify_Action_Func )( GameObject * obj, int action_id, const ActionParamsStruct & params, bool modify_move, bool modify_attack );
	...
	// Weapons
	void	( * Trigger_Weapon_Func )( GameObject * obj, bool trigger, const Vector3 & target, bool primary );
	void	( * Select_Weapon )( GameObject * obj, const char * weapon_name );

	// Custom Script
	void	( * Send_Custom_Event_Func )( GameObject * from, GameObject * to, int type, int param, float delay );
	...
	void	( * Set_Animation_Func )( GameObject * obj, const char * anim_name, bool looping, const char * sub_obj_name, float start_frame, float end_frame, bool is_blended );
	...
	void	( * Stop_Sound_Func )( int sound_id, bool destroy_sound );
	...
	GameObject * ( * Find_Closest_Soldier_Func )( const Vector3 & pos, float min_dist, float max_dist, bool only_human );
	...
	// Explosions
	void	(* Create_Explosion_Func )( const char * explosion_def_name, const Vector3 & pos, GameObject * creator );
	void	(* Create_Explosion_At_Bone_Func )( const char * explosion_def_name, GameObject * object, const char * bone_name, GameObject * creator );

	// HUD
	void	(* Enable_HUD )( bool enable );
	void	(* Mission_Complete )( bool success );

	void	(* Give_PowerUp_Func )( GameObject * obj, const char * preset_name, bool display_on_hud );
	...
	void	(* Set_Innate_Soldier_Home_Location_Func )( GameObject * obj, const Vector3& home_pos, float home_radius );
	...
	void	(* Static_Anim_Phys_Goto_Frame_Func )( int obj_id, float frame, const char * anim_name );
	...
	// Camaera Shakes
	void	(* Shake_Camera_Func)( const Vector3 & pos, float radius, float intensity, float duration );
	...
	// Keys
	void	(* Grant_Key_Func)( GameObject* object, int key, bool grant );
	...
	// Conversation
	int	(* Create_Conversation_Func)( const char *conversation_name, int priority, float max_dist, bool is_interruptable );
	void	(* Join_Conversation_Func)( GameObject * object, int active_conversation_id, bool allow_move, bool allow_head_turn, bool allow_face );
	void	(* Join_Conversation_Facing)( GameObject * object, int active_conversation_id, int obj_id_to_face );
	void	(* Start_Conversation_Func)( int active_conversation_id, int action_id );
	...
	// Apply Damage
	void	(* Apply_Damage_Func)( GameObject * object, float amount, const char * warhead_name, GameObject * damager );
...
*/
class ScriptCommandsClass : public ScriptCommands {
public:
	void Modify_Action(GameObject * obj, int action_id, const ActionParamsStruct & params, bool modify_move = true, bool modify_attack = true) const { Modify_Action_Func(obj, action_id, params, modify_move, modify_attack); }
	void Trigger_Weapon(GameObject * obj, bool trigger, const Vector3 & target, bool primary = true) const { Trigger_Weapon_Func(obj, trigger, target, primary); }
	void Send_Custom_Event(GameObject * from, GameObject * to, int type = 0, int param = 0, float delay = 0.0f) const { Send_Custom_Event_Func(from, to, type, param, delay); }
	void Set_Animation(GameObject * obj, const char * anim_name, bool looping, const char * sub_obj_name = NULL, float start_frame = 0.0F, float end_frame = -1.0F, bool is_blended = false) const { Set_Animation_Func(obj, anim_name, looping, sub_obj_name, start_frame, end_frame, is_blended); }
	...
	void Create_Explosion_At_Bone(const char * explosion_def_name, GameObject * object, const char * bone_name, GameObject * creator = NULL) const { Create_Explosion_At_Bone_Func(explosion_def_name, object, bone_name, creator); }
	void Give_PowerUp(GameObject * obj, const char * preset_name, bool display_on_hud = false) const { Give_PowerUp_Func(obj, preset_name, display_on_hud); }
	void Set_Innate_Soldier_Home_Location(GameObject * obj, const Vector3& home_pos, float home_radius = 999999.0f) const { Set_Innate_Soldier_Home_Location_Func(obj, home_pos, home_radius); }
	void Static_Anim_Phys_Goto_Frame(int obj_id, float frame, const char * anim_name = NULL) const { Static_Anim_Phys_Goto_Frame_Func(obj_id, frame, anim_name); }
	void Static_Anim_Phys_Goto_Last_Frame(int obj_id, const char * anim_name = NULL) const { Static_Anim_Phys_Goto_Last_Frame_Func(obj_id, anim_name); }
	void Add_Objective(int id, int type, int status, int short_description_id, const char * description_sound_filename = NULL, int long_description_id = 0) const { Add_Objective_Func(id, type, status, short_description_id, const_cast<char *>(description_sound_filename), long_description_id); }
	void Shake_Camera(const Vector3 & pos, float radius = 25.0f, float intensity = 0.25f, float duration = 1.5f) const { Shake_Camera_Func(pos, radius, intensity, duration); }
	void Grant_Key(GameObject* object, int key, bool grant = true) const { Grant_Key_Func(object, key, grant); }
	int Create_Conversation(const char * conversation_name, int priority = 0, float max_dist = 0.0f, bool is_interruptable = true) const { return Create_Conversation_Func(conversation_name, priority, max_dist, is_interruptable); }
	void Join_Conversation(GameObject * object, int active_conversation_id, bool allow_move = true, bool allow_head_turn = true, bool allow_face = true) const { Join_Conversation_Func(object, active_conversation_id, allow_move, allow_head_turn, allow_face); }
	void Start_Conversation(int active_conversation_id, int action_id = 0) const { Start_Conversation_Func(active_conversation_id, action_id); }
	void Apply_Damage(GameObject * object, float amount, const char * warhead_name, GameObject * damager = NULL) const { Apply_Damage_Func(object, amount, warhead_name, damager); }
};
...
*/
ScriptCommandsClass	*Get_Script_Commands( void );
