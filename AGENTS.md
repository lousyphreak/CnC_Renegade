...
**ALWAYS** build and run the main executable before returning to the user, to make sure that the port is working and that there are no crashes or major issues for at least 200 seconds. Should **ANY** crash or major issue be found, fix it as part of your task before returning to the user. If the user closes the game, and there are no issues that is also a valid outcome.
...
The original released code can be found at the git tag `original`, so if you need to check how something worked before, you can check out that tag and see the original code.