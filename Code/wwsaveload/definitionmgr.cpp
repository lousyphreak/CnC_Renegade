			...
			if (curr_def != NULL) {
						...
						(twiddle && (curr_def->Get_Class_ID () == CLASSID_TWIDDLERS)))
				{
					...
					if (SDL_strcasecmp (curr_def->Get_Name (), name) == 0) {
						...
						if (!defs) {
							defs=new DynamicVectorClass<DefinitionClass*>;
...
DefinitionMgrClass::Prepare_Definition_Array (void)
{
	if (_DefinitionCount + 1 > _MaxDefinitionCount) {
		...
		DefinitionClass **new_array = new DefinitionClass *[new_size];
	...
	if (!DefinitionHash) DefinitionHash=new HashTemplateClass<StringClass, DynamicVectorClass<DefinitionClass*>*>;
