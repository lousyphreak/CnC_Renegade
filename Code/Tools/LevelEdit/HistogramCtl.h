...
inline void
HistogramCtlClass::Set_Selection (float min, float max)
{
	m_Selection.min = min;
	m_Selection.max = max;
	Set_Dirty (true);
...
inline void
HistogramCtlClass::Get_Selection (float &min, float &max)
{
	min = m_Selection.min;
	max = m_Selection.max;
	return ;
...
inline void
HistogramCtlClass::Set_Dirty (bool dirty)
{
	m_IsDirty = dirty;
	return ;
}
...
inline void
HistogramCtlClass::Allow_Refresh	(bool onoff)
{
