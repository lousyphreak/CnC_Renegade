...
- Runtime bring-up exposed several non-obvious renderer port requirements that do not show up in compile-only work:
  ...
  - On Linux/X11, giving bgfx SDL's X11 `Display*` / `Window` and letting bgfx spawn its own render thread can still crash during Vulkan swapchain creation. The stable startup path here is to use bgfx's documented single-threaded mode on X11 by calling `bgfx::renderFrame()` before `bgfx::init()` on the same thread.
