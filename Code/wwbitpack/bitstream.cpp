...
//-----------------------------------------------------------------------------
void BitStreamClass::Add_Raw_Data(LPCSTR data, uint16_t data_size)
{
	WWASSERT(data != NULL);
...
void BitStreamClass::Add_Wide_Terminated_String(const WCHAR *string, bool permit_empty)
{
	...
	Add(len);
	for (int i = 0; i < len; i++) {
		uint16_t wire_char = static_cast<uint16_t>(string[i]);
		Add(wire_char);
	}
}

//-----------------------------------------------------------------------------
void BitStreamClass::Get_Wide_Terminated_String(WCHAR *buffer, uint16_t buffer_len, bool permit_empty)
{
	WWASSERT(buffer != NULL);
