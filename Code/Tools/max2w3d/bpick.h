...
public:
	...
	*/
	int32_t Filter(INode *node);

	/*
	** From BonePickModeCallback:
	*/
	int32_t HitTest(IObjParam *ip,HWND hWnd,ViewExp *vpt,IPoint2 m,int flags);
	int32_t Pick(IObjParam *ip,ViewExp *vpt);
		
	void EnterMode(IObjParam *ip) { }
	void ExitMode(IObjParam *ip) { }

	PickNodeCallback * GetFilter() {return this;}
	int32_t RightClick(IObjParam *ip,ViewExp *vpt) { return TRUE; }
	...
	*/
	...
	virtual int32_t singleSelect(void) { return SinglePick; }
	virtual int32_t useFilter(void) { return TRUE; }
	virtual int32_t useProc(void) { return TRUE; }
	virtual int32_t doCustomHilite(void) { return FALSE; }
	virtual int32_t filter(INode * inode);
