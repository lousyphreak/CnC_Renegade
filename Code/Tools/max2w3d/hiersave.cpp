...
/* $Header: /Commando/Code/Tools/max2w3d/hiersave.cpp 57    4/03/02 9:38a Moumine_ballo $ */
/*********************************************************************************************** 
...
int HierarchySaveClass::add_node(INode * node,int pidx)
{
	...
	*/
	if (Find_Named_Node(Node[CurNode].Pivot.Name) != -1) {
		...
		LoadString(AppInstance, IDS_ERROR_DUPNAMES, format, MAX_PATH);
		sprintf(buf,format,W3D_NAME_LEN,Node[CurNode].Pivot.Name);
		throw ErrorClass(buf);
	}
//Moumine 3/18/2002    5:01:10 PM
