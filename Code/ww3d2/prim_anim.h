...
LERPAnimationChannelClass<T>::Evaluate (float time)
{
	...
	if (time < this->m_Data[key_count - 1].Get_Time ()) {
		...
		typename PrimitiveAnimationChannelClass<T>::KeyClass *key1 = &this->m_Data[this->m_LastIndex];
		typename PrimitiveAnimationChannelClass<T>::KeyClass *key2 = &this->m_Data[key_count - 1];
