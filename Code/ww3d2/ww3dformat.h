...
** renderer format-code space used by Renegade; we will add new members to
** this list as needed (keeping it in the same order as the original values).
** NOTE: Whenever this is changed, formconv.h/.cpp must be modified as well
** (that contains the code for converting between this and renderer format codes)..
**
